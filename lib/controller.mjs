...
function onOpen(config) {
    ...
    PDFViewerApplication.open(config.document).then(async () => {
        ...
        PDFViewerApplication.pdfViewer.eventBus.on("scrollmodechanged", () => {
            vscode.postMessage({
                type: 'status',
                body: { scrollMode: scrollModeToString(PDFViewerApplication.pdfViewer.scrollMode) }
            })
        });
        ...
        PDFViewerApplication.pdfViewer.eventBus.on("spreadmodechanged", () => {
            vscode.postMessage({
                type: 'status',
                body: { spreadMode: spreadModeToString(PDFViewerApplication.pdfViewer.spreadMode) }
            })
        });
        ...
        PDFViewerApplication.pdfViewer.eventBus.on("scalechanging", () => {
            vscode.postMessage({
                type: 'status',
                body: { zoomMode: PDFViewerApplication.pdfViewer.currentScaleValue }
            })
        });
